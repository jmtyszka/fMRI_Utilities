...
# Prepare eroded brain mask
echo "Preparing eroded brain mask for GRE fieldmap"
bet ${c_fmap_mag} ${tmp_brain} -m
fslmaths ${tmp_mask} -eroF ${c_brain_mask}
rm -rf ${tmp_brain} ${tmp_mask}

# Apply brain mask to GRE fieldmap mag
echo "Applying eroded brain mask to fieldmap magnitude"
fslmaths ${c_fmap_mag} -mas ${c_brain_mask} ${c_fmap_mag_brain}

# Prepare the rad/s fieldmap
echo "Preparing rad/s fieldmap"
fsl_prepare_fieldmap SIEMENS ${c_fmap_phs} ${c_fmap_mag_brain} ${c_fmap_rads} ${dTE}

# Warp the GRE mag and phs fieldmap to distorted EPI space
echo "Warping GRE fieldmap back to distorted EPI space"
fugue -i ${c_fmap_mag} --dwell=${func_esp} --loadfmap=${c_fmap_rads} --unwarpdir=${d2c_unwarp_dir} --nokspace -w ${d_fmap_mag}
